...
回顾第 5 题，CNN 通过"卷积核"提取空间特征，擅长处理图像。

但现实世界中很多数据是**序列**

现在让我们从基础开始，逐步建立对RNN的完整认知：
...
1. **循环神经网络的基本架构**
   - 🔍 思考：为什么叫"循环"？数学公式中的"循环"在哪？RNN和普通神经网络有什么不同？（模型结构受数据特征影响）
...
0. **思考问题**（仅起引导作用，你无需在提交文档中呈现）

   Q1) 为什么 RNN 不能并行？

   Q2) 对LSTM 4 个门的输入输出维度进行推导

   Q3) GRU 相比 LSTM 少了什么？参数量减少多少？

   Q4) Seq2Seq 的 Encoder-Decoder 为什么需要 Context Vector？
...
2. **架构对比分析** 比较RNN、LSTM、GRU三种架构的特点：
   ...
   c) 适用场景分析 - RNN 在 LLM 时代还有用吗？

3. **双向RNN深度分析** 分析双向RNN相比单向RNN的优势和劣势，什么情况下不能使用双向RNN？

</Steps>

> 💪 **挑战自己**：尝试用自己的话来解释这些概念，最好不要生搬概念，就像在向朋友介绍一样！
...
class LSTMCell:
    def __init__(self, input_size, hidden_size):
        ...
        # TODO: 初始化四个门的参数
        # 提示：每个门都需要：
        #   W_x: [input_size, hidden_size]   处理当前输入
        #   W_h: [hidden_size, hidden_size]  处理上一时刻隐藏状态
        #   b:   [hidden_size]               偏置
        # 四个门：遗忘门 f、输入门 i、候选记忆 g、输出门 o
        # 思考：为什么 W 的形状是 [hidden_size, hidden_size + input_size]？
        #      （提示：可以把 h_{t-1} 和 x_t 拼接起来一次性算）
        pass
...
- **重要提醒**：为保证在面试时对RNN具有完整的思路，能够独立应对相关问题，应在尝试手搓核心代码,推导相关公式后再使用AI修改
